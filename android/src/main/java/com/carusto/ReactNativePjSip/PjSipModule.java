...
public class PjSipModule extends ReactContextBaseJavaModule {
    ...
    @ReactMethod
    public void sendInstantMessage(int accountId, String sipServer, String destination, String msgBody,
            Callback callback) {
        String buddy_uri = "<sip:" + destination + "@" + sipServer + ">";
        int callbackId = receiver.register(callback);
        Intent intent = PjActions.sendMessage(callbackId, accountId, buddy_uri, msgBody, getReactApplicationContext());
        getReactApplicationContext().startService(intent);
    }
