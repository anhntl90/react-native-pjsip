...
public class PjActions {
    ...
    public static Intent sendMessage(int callbackId, int accountId, String buddy_uri, String msgBody, Context context) {
        Intent intent = new Intent(context, PjSipService.class);
        intent.setAction(PjActions.ACTION_SEND_MESSAGE);
        intent.putExtra("callback_id", callbackId);
        intent.putExtra("account_id", accountId);
        intent.putExtra("buddy_uri", buddy_uri);
        intent.putExtra("msgBody", msgBody);
        Log.d("ReactNative", "Action sendMessage: accountId" + accountId + " callback_id: " + callbackId
                + " buddy_uri: " + buddy_uri + " msgBody: " + msgBody);
        return intent;
    }
